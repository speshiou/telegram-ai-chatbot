...
def _model_options(_):
    ...
    for key, model in config.DEFAULT_MODELS.items():
        ...
        options.append({"label": label, "value": key, "callback": "set_model|" + key})
    return options
...
def chat_mode_tips(chat_mode, _):
    ...
    if chat_mode is not None and "⚡" in features:
        ...
        text += build_tips(
            ...
            _,
            title=_("<b>How to do instant access?</b>"),
        )
    return text
...
def _menu_page(path: str, menu_data, _):
    ...
    if "options" in data:
        if isinstance(data["options"], dict):
            ...
            for option in data["options"]:
                label = option["label"]
                if (
                    "disable_check_mark" not in data
                    and "value" in data
                    and data["value"] == option["value"]
                ):
                    label = "✅ " + label
                ...
                if "callback" in option:
                    ...
                    callback_value = (
                        option["value"] if option["value"] is not None else ""
                    )
    ...
    keyboard_rows = []
    if len(keyboard) > 0:
        keyboard_rows = build_keyboard_rows(
            keyboard, num_keyboard_cols=num_keyboard_cols
        )
    if len(segs) > 1:
        ...
        keyboard_rows.append(
            [InlineKeyboardButton("< " + _("Back"), callback_data=callback_data)]
        )
    else:
...
def about(_):
    text = _(
        "Hi! My name is Nexia, an AI chatbot powered by OpenAI's GPT, DALL·E and Stable Diffusion models."
    )
    text += "\n\n"
...
def image_menu(_, path=None):
    ...
    for key, value in gen_image_utils.MODELS.items():
        ...
        for size in sizes:
            ...
            size_options.append({"label": label, "callback": callback_data})
